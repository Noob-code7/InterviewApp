...
export const getReport = async (req, res) => {
  ...
  try {
    ...
    answers.forEach((ans) => {
      ...
      if (v) {
        ...
        if (v.emotionProbabilities && typeof v.emotionProbabilities === "object") {
          // Detect vector scale per-answer: current service stores percentages
          // (sum ~100); some legacy records store 0-1 probabilities (sum ~1).
          const probs = v.emotionProbabilities;
          const probSum = Object.keys(emotionTotals).reduce(
            (s, k) => s + (typeof probs[k] === "number" ? probs[k] : 0), 0
          );
          const scale = probSum > 1.5 ? 1 : 100;
          Object.keys(emotionTotals).forEach((key) => {
            if (typeof probs[key] === "number") {
              emotionTotals[key] += probs[key] * scale;
            }
          });
          emotionCount++;
        }
    ...
    Object.keys(emotionTotals).forEach((key) => {
      ...
      // Values are already normalized to a 0-100 percentage scale at accumulation time
