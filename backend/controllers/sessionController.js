...
export const createSession = async (req, res) => {
  ...
  try {
    ...
    // Whitelist candidate-selected subjects (canonical question-bank tags)
    const ALLOWED_SUBJECTS = ['os', 'dbms', 'networking', 'oop', 'se', 'ds']
    const safeSubjects = Array.isArray(subjectsOfInterest)
      ? [...new Set(subjectsOfInterest.filter((t) => ALLOWED_SUBJECTS.includes(t)))]
      : []
    ...
    const session = new Session({
      ...
      subjectsOfInterest: safeSubjects,
...
export const parseResume = async (req, res) => {
  try {
    ...
    try {
      ...
      // Fallback: try /extract-resume-text
      const f2 = new FormData();
      f2.append("file", req.file.buffer, {
        filename: req.file.originalname,
        contentType: req.file.mimetype || "application/octet-stream",
      });
      nlpRes = await axios.post(
        `${NLP_SERVICE_URL}/extract-resume-text`,
        f2,
        {
    ...
    const payload = nlpRes.data?.data || nlpRes.data || {};
...
export const generateProjectFollowUpHandler = async (req, res) => {
  ...
  try {
    ...
    const resolvedFollowUp = followUp?.questionText ? followUp : (followUp?.followUp?.questionText ? followUp.followUp : null);
    if (resolvedFollowUp && resolvedFollowUp.questionText) {
      return sendSuccess(res, { hasFollowUp: true, followUp: resolvedFollowUp }, 200);
    }
