...
const FIXTURES_DIR = path.resolve(__dirname, '../fixtures_30s');
...
async function runBenchmark30s(concurrencyLevel, token, fixtureFiles, transcripts, writingSample) {
  console.log(`\n========================================================================`);
  console.log(`🚀 APPLES-TO-APPLES BENCHMARK: K = ${concurrencyLevel} (5 Questions, 150 Frames)`);
  console.log(`========================================================================`);
  ...
  const createRes = await axios.post(
    `${API_BASE}/sessions`,
    {
      ...
      questionCount: 5,
      candidateName: `Benchmark 30s Candidate K=${concurrencyLevel}`,
  ...
  // 4. Concurrently execute: Writing Test Analysis + 5 Questions (limited by pool)
  ...
  const questionPromises = activeQuestions.map((q, idx) =>
    pool(async () => {
      ...
      // Intra-question: Face + Voice in parallel
      const facePromise = (async () => {
        ...
        try {
          ...
          const fDur = nowMs() - tF;
          deepFaceQuestionTimes.push(fDur);
  ...
  const avgDeepFacePerQuestionMs = deepFaceQuestionTimes.length > 0 ? deepFaceQuestionTimes.reduce((a, b) => a + b, 0) / deepFaceQuestionTimes.length : 0;
  const avgDeepFacePerFrameMs = avgDeepFacePerQuestionMs / 30; // 30 frames per question
  const reportData = reportRes.data.data || {};

  const result = {
    concurrencyLevel: `K = ${concurrencyLevel}`,
    totalWallClockSec: Number((totalWallClockMs / 1000).toFixed(2)),
    avgDeepFacePerQuestionMs: Number(avgDeepFacePerQuestionMs.toFixed(1)),
    avgDeepFacePerFrameMs: Number(avgDeepFacePerFrameMs.toFixed(1)),
  ...
  console.log(`• Avg DeepFace / Question (30f): ${result.avgDeepFacePerQuestionMs} ms`);
  console.log(`• Avg DeepFace / Frame:          ${result.avgDeepFacePerFrameMs} ms`);
...
async function executeApplesToApples() {
  ...
  const fixtureFiles = [
    path.join(FIXTURES_DIR, 'candidate_30s_q1.webm'),
    path.join(FIXTURES_DIR, 'candidate_30s_q2.webm'),
    path.join(FIXTURES_DIR, 'candidate_30s_q3.webm'),
    path.join(FIXTURES_DIR, 'candidate_30s_q4.webm'),
    path.join(FIXTURES_DIR, 'candidate_30s_q5.webm'),
  ];

  const transcripts = [
    "Process isolation in modern operating systems prevents processes from interfering with each other's memory space using hardware MMU, virtual memory paging, and privilege rings.",
  ...
  console.log('========================================================================');
  console.log('🏁 TRUE APPLES-TO-APPLES BENCHMARK (K = 1, 2, 3, 5 | 150 Frames Total)');
  console.log('========================================================================\n');

  const results = [];

  for (const level of [1, 2, 3, 5]) {
    const res = await runBenchmark30s(level, token, fixtureFiles, transcripts, writingSample);
    results.push(res);
    await new Promise((r) => setTimeout(r, 2000));
  }

  console.log('\n========================================================================');
  console.log('📊 150-FRAME REALISTIC BENCHMARK COMPARISON SUMMARY');
  console.log('========================================================================\n');
...
executeApplesToApples().catch((err) => {
